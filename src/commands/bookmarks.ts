    ...
    async (
      ...
      if (!result.success) {
        if (prepared.enabled) {
          ...
          await completeJevCommand({
            ...
            prepared,
            posts: tweets,
            collection: { source: 'bookmarks', status: 'failed', nextCursor: result.nextCursor },
            json: isJson,
            ...
            printOrdinary: () => {
              ctx.printTweetsResult(
                ...
                { json: false, usePagination, emptyMessage },
              );
            },
            collectionError: `Failed to fetch bookmarks: ${result.error}`,
          });
          return;
        }
      ...
      if (!bookmarks || bookmarks.length === 0) {
        if (prepared.enabled) {
          await completeJevCommand({
            ...
            prepared,
            posts: [],
            collection: { source: 'bookmarks', status: 'ok', nextCursor: result.nextCursor },
            json: isJson,
            data: ordinaryTweetsJsonData([], result.nextCursor, usePagination),
            printOrdinary: () => {
              ctx.printTweetsResult(result, { json: false, usePagination, emptyMessage });
            },
          });
          return;
        }
      ...
      const fetchThread = async (tweet: TweetData): Promise<TweetData[] | null> => {
        ...
        try {
          ...
          console.error(`${ctx.p('warn')}Failed to expand thread for ${tweet.id}.`);
          expansionFailed = true;
          return null;
        }
      };
          ...
          if (!alreadyIncluded) {
            ...
            if (parentFromThread) {
              ...
              try {
                ...
                } else if (prepared.enabled) {
                  console.error(`${ctx.p('warn')}Failed to fetch parent tweet ${bookmark.inReplyToStatusId}.`);
                  expansionFailed = true;
                }
              } catch (error) {
                if (!prepared.enabled) {
      ...
      if (prepared.enabled) {
        await completeJevCommand({
          ...
          json: isJson,
          data: ordinaryTweetsJsonData(uniqueTweets, result.nextCursor, usePagination),
          printOrdinary: () => {
            ctx.printTweetsResult(

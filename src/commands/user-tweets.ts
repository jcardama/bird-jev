    ...
    async (
      ...
      if (prepared.enabled) {
        await completeJevCommand({
          ...
          json: isJson,
          data: ordinaryTweetsJsonData(tweets, result.nextCursor, wantsPaginationOutput),
          printOrdinary: () => {
          ...
          collectionError: result.success ? undefined : `Failed to fetch tweets: ${result.error}`,
        });
        return;
      }
